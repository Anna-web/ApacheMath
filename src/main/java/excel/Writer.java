...
public class Writer {
    ...
    public Writer(String filePath, ArrayList<ArrayList<String>> results, Calculator calculator) {
        this.results = results;
        this.cov = calculator.getCovariation();
        this.nameCov = calculator.getCovariationName();
        this.outputPath = "output/output.xlsx";
    }
    ...
    public void writeIntoExcel() throws IOException {
        ...
        XSSFSheet sheetCov = workbook.getSheet("Covariance matrix");
        if (sheetCov == null) {
            sheetCov = workbook.createSheet("Covariance matrix");
        } else {
            clearSheet(sheetCov);
        }
